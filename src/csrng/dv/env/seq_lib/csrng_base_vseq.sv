...
class csrng_base_vseq extends dv_base_vseq #(
  ...
  task wait_cmd_req_done(uint exp_sts=CMD_STS_SUCCESS);
    ...
    csr_spinwait_or_edn_rst_n(.ptr(ral.INTERRUPT_STATE.CS_CMD_REQ_DONE), .exp_data(1'b1));
    ...
    if (app != SW_APP) begin
      ...
      if ((cs_item.acmd == csrng_pkg::GEN) && await_genbits) begin
        for (int i = 0; i < cs_item.glen; i++) begin
          ...
          for (int i = 0; i < csrng_pkg::GENBITS_BUS_WIDTH/AHBDataWidth; i++) begin
  ...
  task check_interrupts(bit [AHBDataWidth-1:0]  interrupts,
                        bit                     check_set,
                        bit [AHBDataWidth-1:0]  clear = '1);
    uvm_reg          csr_intr_state, csr_intr_enable;
    bit [AHBDataWidth-1:0] act_pins;
    bit [AHBDataWidth-1:0] exp_pins;
    bit [AHBDataWidth-1:0] exp_intr_state;
  ...
  local task run_intr_test_vseq(int unsigned num_times);
    ...
    for (int trans = 1; trans <= num_times; trans++) begin
      bit [AHBDataWidth-1:0] num_used_bits;
      bit [AHBDataWidth-1:0] intr_enable_val[$];
