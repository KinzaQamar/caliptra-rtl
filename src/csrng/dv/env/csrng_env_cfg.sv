...
class csrng_env_cfg extends dv_base_env_cfg #(.RAL_T(csrng_reg_block));
  ...
  function mubi4_t get_rand_mubi4_val(int unsigned t_weight,
                                      int unsigned f_weight,
                                      int unsigned other_weight);
    bit[3:0] val;
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(val,
        `DV_MUBI4_DIST(val, t_weight, f_weight, other_weight), ,msg_id)
    return mubi4_t'(val);
  endfunction
  ...
  virtual function void initialize(bit [31:0] csr_base_addr = '1);
    ...
    num_interrupts = ral.INTERRUPT_STATE.get_n_used_bits();
  ...
  task check_internal_state(uint app = SW_APP, bit compare = 0);
    ...
    for (int i = 0; i < RSD_CTR_LEN/AHBDataWidth; i++) begin
      csr_rd(.ptr(ral.INT_STATE_VAL), .value(rdata));
      hw_reseed_counter = (rdata<<AHBDataWidth*i) + hw_reseed_counter;
    end
    for (int i = 0; i < BLOCK_LEN/AHBDataWidth; i++) begin
      csr_rd(.ptr(ral.INT_STATE_VAL), .value(rdata));
      hw_v = (rdata<<AHBDataWidth*i) + hw_v;
    end
    for (int i = 0; i < KEY_LEN/AHBDataWidth; i++) begin
      csr_rd(.ptr(ral.INT_STATE_VAL), .value(rdata));
      hw_key = (rdata<<AHBDataWidth*i) + hw_key;
    end
    csr_rd(.ptr(ral.INT_STATE_VAL), .value(rdata));
